...
static int ls1c_adc_test(int argc,char **argv)
{
	...
	/* 参数范围检查*/
	if(flag_channal > 3) {
		printf( "channal input wrong!   <0~3> \r\n");
		return 0;
	}
	...
	ADC_C_CTL = 0; //&= ~(1 << 7);		//stop all continue adc.
	ADC_CNT = 0x1400010;			//set adc_pre and adc_gap.
	ADC_INTC= 0;  //clear int
	...
	if (flag_t_n == TS_MODE) {	//touch screen mode.
		...
		ADC_DEB_CNT = 0x400;
		ADC_CNT = ADC_CC_EN | ADC_TOUCH_SEL;
		...
		if (flag_s_c == ADC_SINGLE_MODE) {	//adc single test mode.
			...
			printf ("channel %d, single mode, result = 0x%08x !\n", flag_channal, result);
		}
